...
$GLOBALS['_views']["SEM_TREE_GET_DATA_NO_ENTRIES"] = array("pk"=>"sem_tree_id","temp_table_type"=>"HEAP",
							"query"=>"SELECT a.*, c.Name AS studip_object_name
							 FROM sem_tree a LEFT JOIN Institute c ON (a.studip_object_id = c.Institut_id)
							ORDER BY priority");
$GLOBALS['_views']["SEM_TREE_GET_ENTRIES"] = array("pk"=>"sem_tree_id","temp_table_type"=>"HEAP",
...
$GLOBALS['_views']["SEM_TREE_GET_SEMIDS"] = array("pk"=>"seminar_id","temp_table_type"=>"HEAP",
							"query" => "SELECT  b.seminar_id, " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number, " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end FROM seminar_sem_tree b INNER JOIN seminare c ON(b.seminar_id=c.Seminar_id AND c.status IN(&) AND §) WHERE sem_tree_id IN(&) §");
$GLOBALS['_views']["SEM_TREE_GET_SEMDATA"] = array("query" => "SELECT a.seminar_id,IF(b.visible=0,CONCAT(Name, ' "._("(versteckt)")."'), Name) AS Name,username AS doz_uname, Nachname AS doz_name, " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number , " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end
...
$GLOBALS['_views']["SEM_TREE_GET_NUM_SEM"] = array("query" => "SELECT count(DISTINCT(seminar_id)) , " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number, " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end FROM seminar_sem_tree
													LEFT JOIN seminare USING (seminar_id) WHERE sem_tree_id IN(&) §");

$GLOBALS['_views']["SEM_TREE_GET_LONELY_SEM_DATA"] = array("query" => "SELECT d.Seminar_id AS seminar_id,IF(d.visible=0,CONCAT(d.Name, ' "._("(versteckt)")."'), d.Name) AS Name, " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number, " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end ,username AS doz_uname, Nachname AS doz_name
...
$GLOBALS['_views']["SEM_TREE_GET_NUM_LONELY_SEM"] = array("query" => "SELECT COUNT(DISTINCT(b.seminar_id)) AS num_sem , " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number , " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end FROM Institute a LEFT JOIN seminar_inst b USING(Institut_id)
...
$GLOBALS['_views']["SEM_TREE_INS_ITEM"] = array("query" => "INSERT INTO sem_tree (sem_tree_id,parent_id,name,priority,info,studip_object_id,type) VALUES (?,?,?,?,?,?,?)");
$GLOBALS['_views']["SEM_TREE_UPD_ITEM"] = array("query" => "UPDATE sem_tree SET name=?, info=?, type=? WHERE sem_tree_id=?");
...
$GLOBALS['_views']["SEM_TREE_SEARCH_SEM"] = array("query" => "SELECT b.seminar_id, " . $GLOBALS['_views']['sem_number_sql'] . " AS sem_number, " . $GLOBALS['_views']['sem_number_end_sql'] . " AS sem_number_end FROM sem_tree a LEFT JOIN seminar_sem_tree b USING(sem_tree_id)
													LEFT JOIN seminare c USING(seminar_id)
													WHERE § AND NOT ISNULL(b.seminar_id) AND a.name LIKE ? §");
$GLOBALS['_views']["SEM_TREE_CHECK_PERM"] = array("query" => "SELECT inst_perms FROM user_inst WHERE inst_perms='admin' AND user_id=? AND Institut_id=?");
$GLOBALS['_views']["SEM_TREE_SEARCH_ITEM"] = array("query" => "SELECT a.sem_tree_id,a.parent_id FROM sem_tree a LEFT JOIN sem_tree b ON(a.sem_tree_id=b.parent_id) WHERE a.name LIKE ? AND ISNULL(b.sem_tree_id) AND a.sem_tree_id NOT IN(&)");

$GLOBALS['_views']["SEMINAR_SEM_TREE_DEL_RANGE"] = array("query" => "DELETE FROM seminar_sem_tree WHERE sem_tree_id IN (&)");
$GLOBALS['_views']["SEMINAR_SEM_TREE_DEL_SEM_RANGE"] = array("query" => "DELETE FROM seminar_sem_tree WHERE sem_tree_id IN (&) AND seminar_id IN (&)");
$GLOBALS['_views']["SEMINAR_SEM_TREE_DEL_SEMID_RANGE"] = array("query" => "DELETE FROM seminar_sem_tree WHERE seminar_id IN (&)");
$GLOBALS['_views']["SEMINAR_SEM_TREE_INS_ITEM"] = array("query" => "INSERT IGNORE INTO seminar_sem_tree (seminar_id,sem_tree_id) VALUES (?,?)");
$GLOBALS['_views']["SEMINAR_SEM_TREE_GET_IDS"] = array("query" => "SELECT DISTINCT a.sem_tree_id,b.parent_id FROM seminar_sem_tree a INNER JOIN sem_tree b USING(sem_tree_id) WHERE seminar_id=? ORDER BY parent_id,priority");
$GLOBALS['_views']["SEMINAR_SEM_TREE_GET_EXP_IDS"] = array("query" => "SELECT DISTINCT b.sem_tree_id,c.parent_id FROM seminare a LEFT JOIN seminar_sem_tree b USING(seminar_id) LEFT JOIN sem_tree c USING(sem_tree_id) WHERE a.Institut_id=? AND b.sem_tree_id NOT IN(&)");
...
$GLOBALS['_views']["SEM_SEARCH_LECTURER"] = array("query" => "SELECT user_id FROM auth_user_md5 WHERE user_id <> MD5('studygroup_dozent') AND perms = 'dozent' AND (username LIKE ? OR Vorname LIKE ? OR Nachname LIKE ?)");

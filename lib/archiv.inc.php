...
function dump_sem($sem_id, $print_view = false) {
	...
	if ($Modules["schedule"]) {
		...
		$db->query("SELECT *  FROM termine WHERE range_id='$sem_id' AND date_typ IN " . getPresenceTypeClause() . " ORDER BY date");
		if ($db->num_rows()) {
			$dump.="<br>";
		...
		$db->query("SELECT *  FROM termine WHERE range_id='$sem_id' AND date_typ NOT IN " . getPresenceTypeClause() . " ORDER BY date");
		if ($db->num_rows()) {
			$dump.="<br>";
			$dump.="<table width=100% border=1 cellpadding=2 cellspacing=0>";
			$dump .= " <tr><td colspan=2 align=left class=\"topic\">";
